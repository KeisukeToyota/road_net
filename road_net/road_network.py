...
class RoadNetwork(nx.Graph):
    ...
    def __path_result(self, transit_nodes, result, weight):
        ...
        # データの描画
        plt.title("transit_nodes: {}\nresult: {}\nweight: {}".format(transit_nodes, result, weight))
    ...
    def __astar_algorithm(self, start_node, end_node):
        ...
        astar_result: tuple
            探索結果の経路と重みのtuple
        """
        result_route = nx.astar_path(self, start_node, end_node) # A*アルゴリズムによる探索結果の経路
        result_weight = nx.astar_path_length(self, start_node, end_node) # A*アルゴリズムによる探索結果の経路の重み
    ...
    def generate_network(self, node_count):
        ...
        while len(edges) < node_count * 2:
            ...
            if self.__nodes_distance(source[1]['position'], target[1]['position']) <= math.log(node_count)/math.sqrt(node_count):
                edge = (source[0], target[0], self.__nodes_distance(source[1]['position'], target[1]['position']))
            else:
                continue
    ...
    def load_network_csv(self, nodes_csv, edges_csv):
        ...
        for _, row in nodes_df.iterrows():
            self.add_node(int(row.node), position=(row.position_x, row.position_y))
    ...
    def save_network_csv(self, node_file_name, edge_file_name):
        ...
        nodes_list = []
        for node in self.nodes(data=True):
            nodes_list.append([int(node[0]), node[1]['position'][0], node[1]['position'][1]])
        df = pd.DataFrame(nodes_list, columns=['node', 'position_x', 'position_y'])
    ...
    def route_search(self, transit_nodes):
        ...
        return {'transit_nodes':copy_transit_nodes, 'result_route':result_route, 'result_weight':result_weight}
    ...
    def draw_search_route(self, transit_nodes):
        ...
        self.__path_result(result['transit_nodes'], result['result_route'], result['result_weight'])
